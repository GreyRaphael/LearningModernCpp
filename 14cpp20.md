...
class Dynamic2DArray {
   public:
    Dynamic2DArray(size_t rows, size_t cols) : rows_(rows), cols_(cols), data_(rows * cols) {}
...
int main() {
    ...
    for (size_t i = 0; i < rows; ++i) {
        for (size_t j = 0; j < cols; ++j) {
            array(i, j) = static_cast<double>(i * cols + j);
        }
    }
...
optimized version of dynamic 2d array
