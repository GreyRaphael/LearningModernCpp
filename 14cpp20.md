...
contiguous containers (Compatible with `std::span`):
- `int arr[10]`
- `std::array`
- `std::vector`
- `std::string`

Non-Contiguous Containers (Incompatible with `std::span`)
- `std::list`, `sd::forward_list`
- `std::map`, `std::set`
...
// Function to process the first 16 bytes
void processFirst16(std::span<uint8_t> first16) {
    for (auto& byte : first16) {
        byte += 10;  // Example modification: increment each byte by 1
    }
}

// Function to process the last 8 bytes
void processLast8(std::span<uint8_t> last8) {
    for (auto& byte : last8) {
        byte *= 2;  // Example modification: multiply each byte by 2
    }
}
...
int main() {
    ...
    std::span<uint8_t> fullSpan(data);

    // Create sub-spans
    auto first16_span = fullSpan.first<16>();   // First 16 bytes in compile-time
    auto last8_span = fullSpan.subspan(16, 8);  // Last 8 bytes

    // Alternatively, using last<8>()
    // auto last8_span = fullSpan.last<8>();      // Last 8 bytes

    // Or using subspan with runtime parameters
    // auto first16_span = fullSpan.subspan(0, 16);
    // auto last8_span = fullSpan.subspan(16, 8);

    // Process the spans
    processFirst16(first16_span);
    processLast8(last8_span);

    fmt::println("Modified data:{}", data);
}
```

- `std::span` is a non-owning view, so ensure that the original vector (`data` in this case) remains in scope and is not modified in a way that invalidates the spans (e.g., resizing the vector).
- If you want to ensure that a function does not modify the span's elements, use `std::span<const uint8_t>` as the parameter type.
- While `std::span`does not perform bounds checking on element access via `operator[]`, you can use the `at()` member function for bounds-checked access, which throws an exception if out of range.


reimplement with `std::ranges`
...
int main() {
    // Initialize the vector with values 0 to 63
    std::vector<uint8_t> data(24);
    for (uint8_t i = 0; i < 24; ++i) {
        data[i] = i;
    }
    ...
    // Create sub-ranges using ranges views
    auto first16_view = data | std::views::take(16);  // First 16 bytes
    auto last8_view = data | std::views::drop(16);    // Last 8 bytes

    // // Create sub-ranges using ranges::subrange
    // auto first16_view = std::ranges::subrange(data.begin(), data.begin() + 16);
    // auto last8_view = std::ranges::subrange(data.begin() + 16, data.end());

    // Process the sub-ranges
    processFirst16(first16_view);
    processLast8(last8_view);
...
template <typename T>
inline auto parse(const std::string& in_str, T default_value = T{}) noexcept -> T {
    // if parse failed, return unchanged default_value
    // if parse succeeded, return changed default_value
    std::from_chars(in_str.data(), in_str.data() + in_str.size(), default_value);
    return default_value;
}

int main() {
    auto a = parse<float>("a3.1415926");
    std::println("{}", a);  // 0
    auto b = parse<float>("a3.1415926", 6.128f);
    std::println("{}", b);  // 6.128

    auto c = parse<int>("12345");
    std::println("{}", c);  // 12345
    auto d = parse<int>("12345.1");
    std::println("{}", d);  // 12345
    auto e = parse<int>("a1234", 666);
    std::println("{}", e);  // 666
}
```

`std::from_chars` with `std::optional`
...
The `std::remove_cvref` type trait in C++20 removes both `const`/`volatile` qualifiers and `references` from a type, which is similar to what `std::decay_t` does but without converting arrays to pointers or function types to function pointers.
...
void printType() {
    if constexpr (std::is_same_v<T, std::remove_cvref_t<T>>) {
        std::cout << "Type is already decayed.\n";
    } else {
        std::cout << "Type is not decayed.\n";
    }
}

int main() {
    printType<int>();         // Type is already decayed.
    printType<const int&>();  // Type is not decayed.
    printType<int&>();        // Type is not decayed.
    printType<int&&>();       // Type is not decayed.
}
```