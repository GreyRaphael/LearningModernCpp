...
int main() {
    ...
    PrintEx(scores, [](auto const& x) { return std::format("({},{},{})", x.math, x.physics, x.name); });
    // use std::tie
    std::ranges::sort(scores, [](auto const& x, auto const& y) { return std::tie(x.math, x.name[0]) < std::tie(y.math, y.name[0]); });
    PrintEx(scores, [](auto const& x) { return std::format("({},{},{})", x.math, x.physics, x.name); });
};
```
...
int main() {
    ...
    // ranges version:
    std::ranges::transform(prods, begin(prods), [v = 0](const std::string &n) mutable { return Product{n + std::to_string(v++), 1.0}; }, &Product::name_);
    for (auto &p : prods) std::cout << p.name_ << ", ";
}
```
